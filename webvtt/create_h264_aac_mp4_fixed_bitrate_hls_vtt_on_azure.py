...
from bitmovin_api_sdk import AzureInput, AzureOutput
...
from bitmovin_api_sdk import WebVttConfiguration, ChunkedTextMuxing
...
from bitmovin_api_sdk import HlsManifest, HlsVersion, AudioMediaInfo, StreamInfo, SubtitlesMediaInfo
...
TEST_ITEM = "h264-aac-mp4-fixed-bitrate-hls-vtt-azure"
...
def main():
    ...
    azure_input = bitmovin_api.encoding.inputs.azure.create(
        azure_input=AzureInput(
            account_name=AZURE_INPUT_ACCOUNT_NAME,
            account_key=AZURE_INPUT_ACCOUNT_KEY,
            container=AZURE_INPUT_CONTAINER_NAME,
            name='Test Azure Input'))
    azure_output = bitmovin_api.encoding.outputs.azure.create(
        azure_output=AzureOutput(
            account_name=AZURE_OUTPUT_ACCOUNT_NAME,
            account_key=AZURE_OUTPUT_ACCOUNT_KEY,
            container=AZURE_OUTPUT_CONTAINER_NAME,
            name='Test Azure Output'))

    # === Encoding definition ===
    encoding = bitmovin_api.encoding.encodings.create(
        encoding=Encoding(
            name=f'{TEST_ITEM}',
            cloud_region=CloudRegion.AZURE_JAPAN_EAST,
    ...
    # === Input Stream definition for vtt file ===
    vtt_file_input_stream = bitmovin_api.encoding.encodings.input_streams.file.create(
        encoding_id=encoding.id,
        file_input_stream=FileInputStream(
            input_id=azure_input.id,
            input_path=INPUT_VTT_PATH,
            file_type=FileInputStreamType.WEBVTT))
    vtt_input_stream = StreamInput(input_stream_id=vtt_file_input_stream.id)
    ...
    for profile_h264 in encoding_profiles_h264:
        ...
        bitmovin_api.encoding.encodings.muxings.mp4.create(
            ...
            mp4_muxing=Mp4Muxing(
                ...
                filename=f"video_{profile_h264.get('height')}p.mp4",
                fragmented_mp4_muxing_manifest_type=FragmentedMp4MuxingManifestType.HLS_BYTE_RANGES,
    ...
    for profile_aac in encoding_profiles_aac:
        ...
        bitmovin_api.encoding.encodings.muxings.mp4.create(
            ...
            mp4_muxing=Mp4Muxing(
                ...
                filename=f"aac_{profile_aac.get('bitrate')}.mp4",
                fragmented_mp4_muxing_manifest_type=FragmentedMp4MuxingManifestType.HLS_BYTE_RANGES,
                fragment_duration=4000,
                name=f"Audio MP4 Muxing {profile_aac.get('bitrate')/1000:.0f}kbps"))
    ...
    vtt_subtitle_stream = bitmovin_api.encoding.encodings.streams.create(
        ...
        stream=Stream(
            ...
            input_streams=[vtt_input_stream],
            name="Stream WebVTT",
            mode=StreamMode.STANDARD))

    vtt_muxing_output = EncodingOutput(
        output_id=azure_output.id,
        output_path=f"{OUTPUT_BASE_PATH}en",
        acl=[AclEntry(permission=AclPermission.PUBLIC_READ)])
    ...
    bitmovin_api.encoding.encodings.muxings.chunked_text.create(
        ...
        chunked_text_muxing=ChunkedTextMuxing(
            segment_length=10.0,
            segment_naming="en_webvtt_seg_%number%.vtt",
    ...
    hls_manifest = _create_hls_manifest(encoding_id=encoding.id, output=azure_output, output_path=OUTPUT_BASE_PATH)
...
def _create_hls_manifest(encoding_id, output, output_path):
    ...
    # WebVTT manifest definition
    chunked_text_muxing = bitmovin_api.encoding.encodings.muxings.chunked_text.list(encoding_id=encoding_id).items
    subtitle_stream = bitmovin_api.encoding.encodings.streams.get(
        encoding_id=encoding_id, stream_id=chunked_text_muxing[0].streams[0].stream_id)
    vtt_segment_path = _remove_output_base_path(chunked_text_muxing[0].outputs[0].output_path)
    ...
    bitmovin_api.encoding.manifests.hls.media.subtitles.create(
        ...
        subtitles_media_info=SubtitlesMediaInfo(
            name="english - sample",
            encoding_id=encoding_id,
            muxing_id=chunked_text_muxing[0].id,
