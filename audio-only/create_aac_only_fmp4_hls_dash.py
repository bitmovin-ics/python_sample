...
from bitmovin_api_sdk import HlsManifest, HlsVersion, AudioMediaInfo
...
TEST_ITEM = "audio-only-aac-fmp4-hls-dash"
...
def main():
    ...
    encoding = bitmovin_api.encoding.encodings.create(
        encoding=Encoding(
            name=f'Sample Audio-Only AAC Encoding ({TEST_ITEM})',
            cloud_region=CloudRegion.AWS_AP_NORTHEAST_1,
            encoder_version='STABLE'))

    # === IngestInputStream definition for audio (MP3 input) ===
    ...
    # === Audio Codec Configuration, Stream, and Muxing definition ===
    for audio_profile in audio_encoding_profiles:
        ...
        aac_codec = bitmovin_api.encoding.configurations.audio.aac.create(
            aac_audio_configuration=AacAudioConfiguration(
                name=f"AAC {audio_profile.get('bitrate') / 1000:.0f}kbps",
    ...
    # === Manifest Generation (post-encoding) ===
...
def _create_hls_manifest(encoding_id, output, output_path):
    ...
    for muxing in fmp4_muxings.items:
        ...
        if codec.type == CodecConfigType.AAC:
            ...
            # AudioMediaInfo: audio rendition entry in master playlist
