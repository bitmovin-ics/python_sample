...
from bitmovin_api_sdk import Thumbnail, ThumbnailUnit
...
TEST_ITEM = "h264-aac-fmp4-fixed-bitrate-dash-hls-with-stream-condition-with-thumbnail-azure"
...
# Thumbnail position definition. We can add multiple position in seconds. If [10], we only generate a simple thumbnail.
...
def main():
    ...
    for profile_h264 in encoding_profiles_h264:
        ...
        if profile_h264.get('height') == 540:
            ...
            bitmovin_api.encoding.encodings.streams.thumbnails.create(
                encoding_id=encoding.id,
    ...
    dash_manifest = _create_dash_manifest(encoding_id=encoding.id, output=azure_output, output_path=OUTPUT_BASE_PATH)
