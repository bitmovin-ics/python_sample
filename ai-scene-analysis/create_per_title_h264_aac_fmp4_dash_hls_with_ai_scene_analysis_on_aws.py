# Per-Title H.264 + AAC (FMP4) HLS/DASH encoding on AWS with Bitmovin AI Scene Analysis (AISA).
#
# AI Scene Analysis runs at encoding start time and, in addition to the regular HLS/DASH output,
# produces:
#   - asset_description     : a JSON file describing the asset's scenes (written to the output)
...
from bitmovin_api_sdk import Fmp4Muxing, PerTitle, H264PerTitleConfiguration, AutoRepresentation
...
from bitmovin_api_sdk import CodecConfigType, MessageType
from bitmovin_api_sdk import AiSceneAnalysis, AiSceneAnalysisFeatures, AiSceneAnalysisAssetDescription
from bitmovin_api_sdk import AiSceneAnalysisAutomaticAdPlacement, AllSceneBoundaries, AiSceneAnalysisOutputLanguageCodes
from bitmovin_api_sdk import Status, StartEncodingRequest, StartManifestRequest, ManifestGenerator

TEST_ITEM = "per-title-h264-aac-fmp4-ai-scene-analysis-aws"
...
# Languages for the AI-generated scene descriptions (ISO 639-1 codes).
AI_OUTPUT_LANGUAGE_CODES = ["ja", "en"]

bitmovin_api = BitmovinApi(api_key=API_KEY, tenant_org_id=ORG_ID)

# A single Per-Title template stream. The actual rendition ladder is expanded by Bitmovin
# Per-Title at encoding time based on this template.
...
def main():
    ...
    # === Video streams and FMP4 muxings (Per-Title template) ===
    for video_profile in video_encoding_profiles:
        ...
        video_muxing_output = EncodingOutput(
            ...
            output_path=OUTPUT_BASE_PATH + "video/{height}_{bitrate}_{uuid}",
    ...
    # === Audio streams and FMP4 muxings ===
    ...
    # === Start the encoding with Per-Title + AI Scene Analysis ===
    ...
    start_encoding_request = StartEncodingRequest(
        per_title=PerTitle(
            h264_configuration=H264PerTitleConfiguration(
                ...
                codec_max_bitrate_factor=2.0,
                codec_bufsize_factor=4.0,
                codec_min_bitrate_factor=0.5)),
        ai_scene_analysis=AiSceneAnalysis(
            features=AiSceneAnalysisFeatures(
                ...
                asset_description=AiSceneAnalysisAssetDescription(
                    ...
                    outputs=[ai_output]),
                # Insert SCTE cue tags at every detected scene boundary.
                automatic_ad_placement=AiSceneAnalysisAutomaticAdPlacement(
                    all_scene_boundaries=AllSceneBoundaries(
                        is_enabled=True,
                        insert_cue_tags=True)),
                # Languages for the AI-generated descriptions.
    ...
    # === Create and generate HLS/DASH manifests ===
...
def _create_hls_manifest(encoding_id, output, output_path):
    """Create an HLS manifest, adding an entry per FMP4 muxing (audio media / video stream)."""
...
def _create_dash_manifest(encoding_id, output, output_path):
    """Create a DASH manifest with a Period plus video/audio Adaptation Sets and FMP4 representations."""
    manifest_output = EncodingOutput(
        output_id=output.id,
        output_path=output_path,
        acl=[AclEntry(permission=AclPermission.PUBLIC_READ)])
    ...
    for muxing in fmp4_muxings.items:
        ...
        codec = bitmovin_api.encoding.configurations.type.get(configuration_id=stream.codec_config_id)
        segment_path = _remove_output_base_path(muxing.outputs[0].output_path)
        adaptation_set_id = audio_adaptation_set.id if codec.type == CodecConfigType.AAC else video_adaptation_set.id
...
def _execute_hls_manifest_generation(hls_manifest):
    """Start HLS manifest generation and poll until completed or failed."""
...
def _execute_dash_manifest_generation(dash_manifest):
    """Start DASH manifest generation and poll until completed or failed."""
...
def _wait_for_hls_manifest_to_finish(manifest_id):
    """Poll HLS manifest creation status every 5 seconds."""
    time.sleep(5)
...
def _wait_for_dash_manifest_to_finish(manifest_id):
    """Poll DASH manifest creation status every 5 seconds."""
    time.sleep(5)
...
def _remove_output_base_path(text):
    """Strip the OUTPUT_BASE_PATH prefix to produce a manifest-relative segment path."""
