...
from bitmovin_api_sdk import HttpsInput, S3Output
...
TEST_ITEM = "dolbyvision-dolbyatmos-adm-hls-dash-fmp4-https-input"
...
def main():
    ...
    # === Input Stream definition (video x 1, audio x 2 (primary and secondary) ) ===
    ...
    for encoding_config in audio_encoding_configs:
        ...
        audio_muxing_output = EncodingOutput(output_id=s3_output.id,
                                             output_path=OUTPUT_BASE_PATH + "audio/primary/{}".format(encoding_profile.get('bitrate')),
                                             acl=[AclEntry(permission=AclPermission.PUBLIC_READ)])
        bitmovin_api.encoding.encodings.muxings.fmp4.create(
            encoding_id=encoding.id,
            fmp4_muxing=Fmp4Muxing(
