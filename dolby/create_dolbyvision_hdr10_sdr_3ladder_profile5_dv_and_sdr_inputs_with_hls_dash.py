"""Dolby Vision input + separate SDR input -> DV / HDR10 / SDR, multi-encode + combined manifest.

Variant of the single-input 3-ladder sample. Here SDR is NOT tone-mapped from the Dolby
Vision input; instead it is encoded from a dedicated, separately-graded SDR (BT.709)
mezzanine. Use this when you have a hand-graded SDR master and want it to drive the SDR
ladder rather than relying on automatic HDR-to-SDR conversion.

Each dynamic range is produced by its OWN encoding, and a single combined HLS + DASH manifest
is generated afterwards spanning all of them.
...
WHY MULTI-ENCODE:
  ...
  *within a single encoding* (API error code 2065: "It's not possible to mix Dolby Vision,
  ...
  each format gets its own encoding:

  1. DV encoding    — Dolby Vision Profile 5 ladder (`DOLBY_VISION_PROFILE_5`) <- DV input.
     Pure DV (IPT-PQ-c2), not HDR10-compatible. Atmos + AAC audio live here.
  2. HDR10 encoding — HDR10 ladder (`HDR10`, BT.2020 / PQ / 10-bit) <- DV input.
  3. SDR encoding   — SDR ladder (`SDR`) <- dedicated SDR mezzanine (IngestInputStream,
     VIDEO_RELATIVE). No tone mapping; the SDR master is encoded as-is.
...
NOTE: the DV and SDR mezzanines should be the same content at the same frame rate / duration
so the ABR ladders line up across encodings.
"""
...
TEST_ITEM = "dolbyvision-hdr10-sdr-3ladder-profile5-dv-and-sdr-inputs-hls-dash-fmp4"
...
# Dolby Vision video + sidecar metadata. Feeds the DV Profile 5 and HDR10 encodings.
...
# Dedicated SDR (BT.709) video mezzanine — drives the SDR encoding (no HDR-to-SDR conversion).
# Provide a separately-graded SDR master here.
...
# SDR ladder — encoded from the dedicated SDR (BT.709) input. Lives in its own encoding.
...
def _create_dv_encoding(s3_input, s3_output):
    ...
    audio renditions, from the Dolby Vision input."""
...
def _create_sdr_encoding(s3_input, s3_output):
    """Encoding holding the SDR video ladder, encoded from the dedicated SDR (BT.709) mezzanine
    (no tone mapping). Separate input AND separate encoding."""
    encoding = bitmovin_api.encoding.encodings.create(
        encoding=Encoding(
    ...
    # Dedicated SDR video input (plain ingest, not a Dolby Vision input stream).
    sdr_video_ingest = bitmovin_api.encoding.encodings.input_streams.ingest.create(
        encoding_id=encoding.id,
        ingest_input_stream=IngestInputStream(
            input_id=s3_input.id,
            input_path=SDR_INPUT_PATH,
            selection_mode=StreamSelectionMode.VIDEO_RELATIVE,
            position=0))
    video_input_stream = StreamInput(input_stream_id=sdr_video_ingest.id)
    ...
    for profile in profiles:
        h265_codec = bitmovin_api.encoding.configurations.video.h265.create(
            h265_video_configuration=H265VideoConfiguration(
                ...
                # Profile 5 = pure DV; HDR10 = dedicated HDR10; SDR = encoded from the SDR input.
