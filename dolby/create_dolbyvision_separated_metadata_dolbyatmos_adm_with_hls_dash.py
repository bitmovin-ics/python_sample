...
from bitmovin_api_sdk import IngestInputStream, StreamSelectionMode, Stream, StreamInput, MuxingStream, StreamMode
from bitmovin_api_sdk import DolbyVisionMetadataIngestInputStream, DolbyVisionMetadata, DolbyVisionMetadataSource, DolbyVisionProfile
...
TEST_ITEM = "dolbyvision-separated-metadata-dolbyatmos-adm-hls-dash-fmp4"
...
S3_METADATA_INPUT_ACCESS_KEY = '<INSERT_YOUR_ACCESS_KEY>'
S3_METADATA_INPUT_SECRET_KEY = '<INSERT_YOUR_SECRET_KEY>'
S3_METADATA_INPUT_BUCKET_NAME = '<INSERT_YOUR_BUCKET_NAME>'
...
def main():
    ...
    s3_input = bitmovin_api.encoding.inputs.s3.create(
        s3_input=S3Input(
            ...
            name='Test S3 Input'))
    s3_input_for_metadata = bitmovin_api.encoding.inputs.s3.create(
        s3_input=S3Input(
            access_key=S3_METADATA_INPUT_ACCESS_KEY,
            secret_key=S3_METADATA_INPUT_SECRET_KEY,
            bucket_name=S3_METADATA_INPUT_BUCKET_NAME,
            name='Test S3 Input'))
    ...
    # === Input Stream definition (video x 1, audio x 1) ===
    video_ingest_input_stream = bitmovin_api.encoding.encodings.input_streams.ingest.create(
        ...
        ingest_input_stream=IngestInputStream(
            ...
            input_path=DOLBY_VISION_INPUT_PATH,
            selection_mode=StreamSelectionMode.VIDEO_RELATIVE,
            position=0
        )
    )
    dolby_vision_metadata_ingest_input_stream = bitmovin_api.encoding.encodings.input_streams.sidecar.dolby_vision_metadata_ingest.create(
        encoding_id=encoding.id,
        dolby_vision_metadata_ingest_input_stream=DolbyVisionMetadataIngestInputStream(
            input_id=s3_input_for_metadata.id,
            input_path=DOLBY_VISION_INPUT_METADATA
        )
    )
    ...
    for encoding_config in video_encoding_configs:
        ...
        bitmovin_api.encoding.encodings.streams.hdr.dolby_vision.create(
            encoding_id=encoding.id,
            stream_id=encoding_config['h265_stream'].id,
            dolby_vision_metadata=DolbyVisionMetadata(
                profile=DolbyVisionProfile.DVHE_05,
                metadata_source=DolbyVisionMetadataSource.INPUT_STREAM,
                metadata_input_stream_id=dolby_vision_metadata_ingest_input_stream.id
            )
        )
