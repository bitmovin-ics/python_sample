...
TEST_ITEM = "per-title-h265-aac-fmp4-dash-hls-aws"
...
def main():
    ...
    for profile_h265 in encoding_profiles_h265_pertitle:
        ...
        # Single FMP4 muxing serves both DASH and HLS (CMAF). {uuid} keeps per-rendition paths unique.
    ...
    # === Create + generate DASH and HLS manifests (both reference the same FMP4 muxings) ===
...
def _create_dash_manifest(encoding_id, output, output_path):
    ...
    renditions expanded by the encoder), routed to the matching adaptation set."""
...
def _create_hls_manifest(encoding_id, output, output_path):
    """HLS manifest (fMP4 / CMAF). Adds one entry per non-template FMP4 muxing (audio media +
    Per-Title video variant streams expanded by the encoder)."""
    manifest_output = EncodingOutput(
        output_id=output.id,
        output_path=output_path,
        acl=[AclEntry(permission=AclPermission.PUBLIC_READ)])
